...
def main(args=None):
    ...
    print(f'\nProcessing complete. Output saved to: {save_dir}')
    if unprocessed_image:
        print(f'Unprocessed images: {len(unprocessed_image)}')
        for img_path in unprocessed_image:
            print(f'  - {img_path}')

    return unprocessed_image, unprocessed_image_size, dic
...
def get_image_pair(args, path, dic: dict):
    ...
    return generated_uuid, img_lq, img_gt
